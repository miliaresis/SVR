...
def print2dscattersHLATLON_LST(RLST, data, x, f):
    """Print H, LAT, LON versus LST combinations of 2-d scatterograms """
    import matplotlib.pyplot as plt
    print('2d visualization of the H, LAT, LON versus RLST feature space')
    f.write('\n 2d visualization of the H, LAT, LON versus RLST feature space')
    k = 0
    xx = ['H', 'Lat', 'Lon']
    for i in range(1, 4):
        for j in range(RLST.shape[1]):
                ...
                plt.xlabel(xx[i-1])
                plt.ylabel(x[j])
                plt.scatter(data[:, i], RLST[:, j], s=3, color='b', marker='+')
                plt.savefig('Scat_RLSTHLATLON'+str(k) + '.png',  dpi=300)
                f.write('\n    ' + 'Scat_RLSTHLATLON' + str(k) + '.png')
                plt.show(1)
    plt.close("all")
...
def RLST_crosscorrelation(data, RLST, Labelmonth1, f):
    ...
    if Dyesno2 == 'Y' or Dyesno2 == 'y':
        Ids, H, LAT, LON, LST = create_data_files(data)
        print('CROSS CORRELATION (H, Lat, Lon, RLST)')
        data2 = np.zeros(shape=(RLST.shape[0], RLST.shape[1]+3))
        data2[:, 0] = data[:, 1]
        data2[:, 1] = data[:, 2]
        data2[:, 2] = data[:, 3]
        data2[:, 3:data2.shape[1]] = RLST
        crosscorrelation = crosscorrelate(data2)
        printRLST_correlation(crosscorrelation, Labelmonth1)
        f.write('\n Compute cross correlation matrix for RLST data ')
        f.write('\n   Write cross correlation matrix to RLST_correlation.xlsx')
